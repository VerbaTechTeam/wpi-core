...
class LoginServiceImplTest {
    ...
    @Test
    @DisplayName("Should rethrow DeserializationException from deserializer without wrapping")
    void shouldPassthroughDeserializationException() {
        DeserializationException original = new DeserializationException("already wrapped", new Exception("root"));
        LoginServiceImpl instance = instanceWithDeserializer(new ResponseDeserializer() {
            @Override
            public <R> R deserialize(String responseBody, Class<R> type) throws Exception {
                throw original;
            }
        });

        DeserializationException thrown = assertThrows(
                DeserializationException.class,
                () -> instance.login(USERNAME, PASSWORD)
        );
        assertAll(
                () -> assertSame(original, thrown, "DeserializationException must not be re-wrapped"),
                () -> assertNull(AuthorizationHolder.get())
        );
    }

    @Test
    @DisplayName("Should wrap response body exception as RuntimeException")
    void shouldWrapResponseBodyExceptionAsRuntimeException() {
        Exception responseException = new Exception("network error");
        LoginServiceImpl instance = new LoginServiceImpl(
                null,
                AuthorizationHolder::get,
                _ -> () -> new Response(null, responseException),
                new ResponseDeserializer() {
                    @Override
                    public <R> R deserialize(String responseBody, Class<R> type) {
                        return fail("Deserializer should not be called when response body throws");
                    }
                }
        );
        ...
        assertAll(
                ...
                () -> assertSame(responseException, thrown.getCause()),
                () -> assertNull(AuthorizationHolder.get())
        );
    }

    @Test
    @DisplayName("Should clear authorization when DeserializationException propagates (it is a RuntimeException)")
    void shouldClearAuthorizationOnDeserializationException() {
        LoginServiceImpl instance = instanceWithDeserializer(new ResponseDeserializer() {
            @Override
            public <R> R deserialize(String responseBody, Class<R> type) throws Exception {
                throw new DeserializationException("parse error", new Exception());
            }
        });
        AuthorizationHolder.authorize("Basic", "temp-token");

        assertThrows(DeserializationException.class, () -> instance.login(USERNAME, PASSWORD));
        assertNull(AuthorizationHolder.get());
    }

    @Test
    @DisplayName("Should clear authorization when agent throws a generic exception")
    void shouldClearAuthorizationOnAgentException() {
        LoginServiceImpl instance = new LoginServiceImpl(
                null,
                AuthorizationHolder::get,
                ...
                new ResponseDeserializer() {
                    @Override
                    public <R> R deserialize(String responseBody, Class<R> type) {
                        return fail("Deserializer should not be called");
                    }
                }
        );
